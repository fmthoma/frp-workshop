...
isAlive state = not wallCollision && not selfCollision
  where
    hd@(Point x y) : tl = points (snake state)
    wallCollision = x < 0 || x > 19 || y < 0 || y > 19
    selfCollision = hd `elem` tl
...
createPage window = do
    ...
    elemGrid <- flexboxGrid
        [ [ col 12 elemCanvas ]
        , [ col 6 elemScore, col 6 btnReset ] ]
...
setup window = mdo
    ...
    let eStep = step <$ whenE (isAlive <$> bApp) eClock
